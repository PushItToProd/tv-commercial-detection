...
The `nascar_on_nbc` profile covers NBC Sports Cup coverage on **both NBC and USA Network** — the same production and the same "NASCAR NON STOP" break, only the corner bug differs. It checks the side-by-side banner in the upper left (`ad`), then any of the network bugs in the upper right (`content`), then the audio sensor, then falls through to `prompt_nbc.txt`. It skips the LLM quick check the other profiles run: over both annotated NBC/USA recordings the quick check never changed a verdict the full prompt would have reached, and with OpenCV and the audio sensor settling most ads first, it rejected too few of the remaining frames to pay for its own request (see `server/experiments/quick_check_logprobs/README.md`).
...
Note that the LLM fallback is weak on pre-race paddock and driver-intro content: `prompt_nbc.txt` calls a frame with no scoring strip or network bug an ad, and pre-race features carry neither. The corner-bug checks run first specifically because they are far more reliable for that material.
